...
//-----------------------------------------------------------------------------
crow_gardner::crow_gardner(const char* Name, double CL, int Type) :
  TNamed(Name,Name) {
...
//-----------------------------------------------------------------------------
// see, if can move to the right to minimize the overcoverage
//-----------------------------------------------------------------------------
  while (imax < MaxNx) {
...
//-----------------------------------------------------------------------------
void crow_gardner::make_belt_hist(int NObs) {
  Belt_t* belt;

  if (NObs == -1) belt = &fBelt0;
  else            belt = &fBelt[NObs];
  ...
  for (int ix=0; ix<MaxNx; ix++) {
    ...
    if (belt->fSign[ix][1] > 0) {    
      belt->fLo->SetBinContent(ix+1,belt->fSign[ix][0]);
      belt->fHi->SetBinContent(ix+1,dx);
    }
  }

  belt->fLo->GetXaxis()->SetTitle("N");
  belt->fLo->GetYaxis()->SetTitle("#mu_{S}");
  
  belt->fLo->SetLineColor(belt->fFillColor);
  belt->fLo->SetLineColor(belt->fFillColor);

  belt->fHi->SetFillStyle(belt->fFillStyle);
  belt->fHi->SetFillColor(belt->fFillColor);
  belt->fHi->SetLineColor(belt->fFillColor);

  belt->fStack = new THStack(Form("hs_%s_%02i",GetName(),NObs),belt->fHi->GetTitle());
  belt->fStack->Add(belt->fLo);
  belt->fStack->Add(belt->fHi);
}
