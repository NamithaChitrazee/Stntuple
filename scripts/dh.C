...
int init_chain(TChain* chain, int JobNumber, TString& output_dir, TString& book, TString& dataset_id);
...
// If flags&1 then read existing AAA files and continue from that point
			   ...
			   const char* dataset = NULL, int flag=0)
 {
   ...
   if(flag&0x1) {
     ...
     //while (fscanf(cat,"%s\n",line) > 0) {
     while (fgets(line,1000,cat) != nullptr) {
       if(line[0]!='#' && line[0]!='<') {
     ...
     while (fgets(line,1000,fil) != nullptr) {
       if(line[0]!='#' && line[0]!='<') {
...
//
// request file (generated by Stntuple/scripts/make_concat_requests.rb contains 
// all necessary information in it
// it has to be loaded by TInterpreter::LoadMacro() prior to loading dh.C
// output_dir is the remote dir to where the file has to be moved
// assume that n_concatenation_requests() function is also loaded
  ...
  char output_file[200], pid[100], ch_cmd;
  ...
  gInterpreter->ProcessLine(Form("n_concatenation_jobs((int*)%p)",&j2));
  j2 = j2+1;
  ...
  printf(">>>> processing requests: j1=%i j2=%i\n",j1,j2);

  for (int j=j1; j<j2; j++) {

    printf(">>>> processing request: j=%i\n",j);

    TChain*    chain = new TChain("STNTUPLE");

    // init_chain(chain,j,output_dir,book,dataset_id);
    cmd = Form("init_chain((TChain*)%p,%i,(TString*)%p,(TString*)%p,(TString*)%p);",chain,j,&output_dir,&book,&dataset_id);
...
//    om->SetMaxFileSize(1800);
    ...
    delete x;
//-----------------------------------------------------------------------------
// done with concatenation, file resides in a local directory, rename it
//-----------------------------------------------------------------------------
    int rc = catalog_file(output_file,111,output_dir,
			  dataset_id.Data(),book.Data());

    printf(" --- job_number = %5i dh::concatenate: return code RC=%i\n",j,rc);
