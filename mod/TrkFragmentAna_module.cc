...
//-----------------------------------------------------------------------------
// compared to _0, swap lanes 3 and 4, and in the new lane3 swap lines 1 and 3
    int adc_index_1[96] = {
      91, 85, 79, 73, 67, 61, 55, 49,
      43, 37, 31, 25, 19, 13,  7,  1,
      90, 84, 78, 72, 66, 60, 54, 48,
...
void TrkFragmentAna::book_histograms(int RunNumber) {
  ...
  _Hist.frag.dt0r_vs_ch[0] = frag_dir.make<TH2F>("dt0r_vs_ch_0", Form("run %06i: dt0r vs ch[0]",RunNumber),  100,0.,100.,5000,-25000,25000);
  _Hist.frag.dt1r_vs_ch[0] = frag_dir.make<TH2F>("dt1r_vs_ch_0", Form("run %06i: dt1r vs ch[0]",RunNumber),  100,0.,100.,5000,-25000,25000);

  _Hist.frag.dt0r_vs_ch[1] = frag_dir.make<TH2F>("dt0r_vs_ch_1", Form("run %06i: dt0r vs ch[1]",RunNumber),  100,0.,100.,20000,-40,60);
  _Hist.frag.dt1r_vs_ch[1] = frag_dir.make<TH2F>("dt1r_vs_ch_1", Form("run %06i: dt1r vs ch[1]",RunNumber),  100,0.,100.,20000,-40,60);
...
//-----------------------------------------------------------------------------
  ...
  if      (rn == 281) {                 // 60 kHz
    _freq  = f0/(pow(2,9)+1);
    offset = 15030;                     // undefined
  }
  else if (rn == 105023) {              // 250 kHz
    _freq  = f0/(pow(2,7)+1);
    offset = 0;                         // undefined - no hits in lanes 2 and 3 (HV)
  }
  else if (rn == 105026) {              //  60 kHz
    _freq  = f0/(pow(2,9)+1);
    offset = 10400;
  }
  else if (rn == 105038) {              //  60 kHz
    _freq  = f0/(pow(2,9)+1);
    offset = 7965;
  }
  else if (rn == 105041) {              //  60 kHz
    _freq  = f0/(pow(2,9)+1);
    offset = 1128;
  }
  else if (rn == 105042) {              //  60 kHz
    _freq  = f0/(pow(2,9)+1);
    offset = 1128;
  }
  else if (rn == 105043) {              //  60 kHz   the offset from the histogram was 1127
    _freq  = f0/(pow(2,9)+1);
    offset = 1128;
  }
  else if (rn == 105044) {              //  60 kHz
    _freq  = f0/(pow(2,9)+1);
    offset = 1129;
  }
                                        // in nanoseconds
...
// there could be a confusion between the HV lanes
//-----------------------------------------------------------------------------
  for (int i=0; i<kNChannels; i++) {
    if (_adc_index_0[i] < 48) _gen_offset[i] = 0;
    else                      _gen_offset[i] = offset+20;
  }
...
//-----------------------------------------------------------------------------
      int t0r = correctedTDC(_data.channel[_referenceChannel].hit[0]->TDC0());
      int t1r = correctedTDC(_data.channel[_referenceChannel].hit[0]->TDC1());

      for (int i=0; i<kNChannels; i++) {
        int nh = _data.channel[i].nhits;
        _Hist.channel[i].nhits->Fill(nh);
        if (nh > 0) {
          int t0 = correctedTDC(_data.channel[i].hit[0]->TDC0());
          int t1 = correctedTDC(_data.channel[i].hit[0]->TDC1());
          
          float dt_over_2(_dt/2);   // convert from sec to ns
          ...
          _Hist.frag.dt0r_vs_ch[0]->Fill(i,dt0r);
          _Hist.frag.dt1r_vs_ch[0]->Fill(i,dt1r);

          _Hist.frag.dt0r_vs_ch[1]->Fill(i,dt0r_c);
          _Hist.frag.dt1r_vs_ch[1]->Fill(i,dt1r_c);
